...
async def create_book(book: Book, session: AsyncSession):
    """
    Create a new book title and one default physical copy.
    """
...
async def create_books_bulk(books: list[Book], session: AsyncSession):
    """
    Create many books in one request and return created books plus any failures.
    """
    created_books = []
    failed_books = []
    ...
    for index, book in enumerate(books, start=1):
        try:
            created_book = await create_book(book, session)
...
async def get_books(
    ...
    if tags:
        ...
        for t in tags:
            query = query.where(Book.tags.contains(t))
...
async def get_suggestions(session: AsyncSession, q: Optional[str] = None, limit: int = 10):
    ...
    query = select(Book).where(
        (Book.title.contains(q)) | (Book.author.contains(q)) | (Book.isbn.contains(q))
    ).limit(limit)
